...
export const TrazeniProizvod = ({ onSearch }) => {
    ...
    return (
        <Layout onSearch={onSearch}>  
            <h1 className="mt-6 mx-auto max-w-2xl px-4 py-10 sm:px-6  lg:max-w-7xl lg:px-8 font-bold text-2xl  mb-0 tracking-tight text-textBoja ">Rezultati pretrage za: "{searchTerm}"</h1>
                        ...
                        <div
                            ...
                            {trenutniProizvodi.map((proizvod) => (
                                <a key={proizvod.id} href={`/${proizvod.kategorija}/${proizvod.id}`} className="group">
                                    ...
                                    <p className="mt-1 text-lg font-medium text-gray-900">{proizvod.cijena}</p>
                                </a>
                            ))}
                            </div>
                    ) : (
